 ...
 # Returns the list of management rules for the given user (The user appears either as a Principal, or as a Role in the Rules)
  def self.user_action_actor_list(user, actor, action)
    ...
    if actor == 'sender'
     ...
     # For each project
    leave_management_project_list.find_each do |project|
  ...
  def self.leave_manages_recursive(to_check = [], checked = [])
      ...
      (to_check - checked).each do |user|
        to_be_checked_next << user.leave_manages(true)
        checked << user
      end

      unless to_be_checked_next.empty?
        self.leave_manages_recursive(to_be_checked_next.flatten, checked)
      else
        return checked
      end
